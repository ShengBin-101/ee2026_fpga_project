...
module Top_Student (
    ...
    group_project (basys_clk , JXADC , game, ticking);
    Game_tone_sounds music (basys_clk, JB, SW [1:0] , led_volume , playing, zpos, new_event);
    ...
    always @(posedge basys_clk) begin
        ...
        if (board_type == 1) begin
            ...
            if (game_state == 2'b01) begin
                ...
                an <= an_password;
                play <= 5;
                if (counter < 15 * `Sec) begin
                tick <= 1;
                end else if (counter < 30 * `Sec) begin
                tick <= 2;
                end else if (counter < 45 * `Sec) begin
                tick <= 3;
                end else begin
                tick <= 4;
                end
                ...
                if (counter >= 60 * `Sec) begin
                counter <= 0;
                game_state <= 2'b11;
                end
                oled_data_bomb <= `ORANGE;
                led_game[13:0] <= password_one;   
